...
**Depends on:** every `@launchkit/*` package (`cli`, `proxy`, `harnesses`, `config`, `sessions`, `secrets`, `ipc`, `ui`, `types`, `utils`)md.
...
- The system tray (`src/gui/tray.ts`) is built from the PURE `buildTrayMenu` descriptor (`src/gui/tray-menu.ts`); the native Electrobun Tray lives only behind the `MountTrayDeps.createTray` seam, and a Launch click reuses the same `ctx.terminal.launch` (embedded-terminal) path as the IPC `launchHarness` handler — the manager owns session creation, so the tray no longer calls `ctx.sessions.create` itself. (The headless `ctx.launch` + inherit-stdio spawner is now CLI-only.)
