# Root `CLAUDE.md` — content to create

`phase0` creates `/CLAUDE.md` at the repo root with the content below (verbatim is fine; keep it short and rule-dense). It is the always-on contract for any agent working in this repo.
...
Keep this file authoritative but thin: it points to `build-plan/01-conventions/*` for detail rather than duplicating it (DRY). When conventions change, update the convention file, not this list.
