...
- `@launchkit/platform` — pure OS detection + idiomatic per-OS path resolution + small platform helpers (depends on nothing)
- `@launchkit/brand` — brand identity source of truth: pure `LaunchKitMark` + canonical tokens/fonts/raster assets (depends on react only)
