...
describe("createRealGateway", () => {
  it("maps streamText text deltas to normalized text-delta and finish events", async () => {
      ...
      doStream: async () => ({
        stream: simulateReadableStream({
          chunks: [
            ...
            { type: "finish" as const, finishReason: "stop" as const, usage: { promptTokens: 1, completionTokens: 2 } },
          ],
        }),
    ...
    const req: NormalizedRequest = { model: "m", messages: [{ role: "user", content: "hi" }], stream: true }
