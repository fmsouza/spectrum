...
**Depends on:** `@launchkit/types`, `@launchkit/utils`, `@launchkit/agent-events`, `@launchkit/brand` (prop shapes, pure formatting, and the canonical event/view-state vocabulary only; never `ipc`/`config`/`proxy`/`sessions`/`harnesses`).
